	...
	def record_rfid(self, request, id_usuario):
		...
		try:
			...
			if request.method != 'POST':
				...
				r = requests.get('http://192.168.0.51', headers={'id_usuario': str(id_usuario)})
    ...
    def salvar_pacote_por_usuario_contaazul(id_contaazul, array_id_pacotes_por_usuario, id_venda, data_venda, quantity):
    	...
    	for id_pacote_por_usuario in array_id_pacotes_por_usuario:
    		...
    		if not pacote_por_usuario_database:
    			pacote_por_usuario = PacotePorUsuario(usuario=usuario_espaco,pacote=pacote,ativo=False,data_ativacao=None,data_encerramento=None,id_venda_contaazul=id_venda, quantidade=int(float(quantity)))
